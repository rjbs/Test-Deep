...
=head3 superbagof(@elements)

=head3 subbagof(@elements)

=head3 supersetof(@elements)

=head3 subsetof(@elements)
...
These are just special cases of the Set and Bag comparisons so the also give
you an add() method and they also have the same limitations when using
...
value of an overloaded reference.
