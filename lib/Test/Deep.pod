...
=head3 $ok = eq_deeply($got, $expected, $name)

This is the same as cmp_deeply() it just returns true or false, however it
never outputs any diagnostics or talks to L<Test::Builder>, so you can use it
in a normal program, rather than in a test script.
...
=head3 num($number, $mode)

$number is a number.

This will check C<$got_v == $number>, even if $got_v is a ref. It is useful
...
"12blah" into 12. You may not want this. If you have version 1.10 or higher
of L<Scalar::Util> then you can pass in C<$mode> as a second argument to
number(). C<$mode> can be "loose" or "strict". In strict mode, "12blah" will
not be considered equal to 12.

If you cannot install the correct version of Scalar::Util then you can
usually just use the string() comparison to be more strict. This will work
fine for almost all situations, however it will not work when <$got_v> is an
overloaded value who's string and numerical values differ.
